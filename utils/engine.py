...
from .utils import AveragePrecisionMeter, Warp, time_str
...
class Engine(object):
    def __init__(self, state={}):
        ...
        if self._state('crop_size') is None:
            self.state['crop_size'] = 224
        ...
        if self._state('visdom_port') is None:
            self.state['visdom_port'] = 8097

        if self._state('numclass') is None:
            self.state['numclass'] = 2
        
        if self._state('logger_title') is None:
            self.state['logger_title'] = 'Hao-Sensitive-Exp'
        ...
        self.state['logger'] = MeterLogger(
            server='localhost', env='Defect_NEW',port=self.state['visdom_port'], nclass=self.state['numclass'], title=self.state['logger_title'])
        # display parameters
    ...
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        # online logger
        self.state['logger'].update_loss(self.state['loss'].cpu(), meter='loss')
        self.state['logger'].update_meter(self.state['output'].cpu(), target_var.cpu(), meters={'map'})
    ...
    def init_learning(self, model, criterion):
        ...
        #         transforms.Resize(self.state['image_size']),
        #         transforms.RandomCrop(self.state['crop_size']),
    ...
    def train(self, data_loader, model, criterion, optimizer, epoch):
        ...
        for i, (input, target) in enumerate(data_loader):
            ...
            # self.state['logger'].print_meter(mode="Train", iepoch=self.state['epoch'])
    ...
    def save_checkpoint(self, state, is_best, filename='checkpoint.pth.tar'):
        if self._state('save_model_path') is not None:
            filename_ = filename
            _time_str = time_str()
            # origin_filename = _time_str+'-checkpoint.pth'
            # soft_link = os.path.join(self.state['save_model_path'], filename_)
...
class MulticlassEngine(Engine):
    ...
    def on_end_epoch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        if display:
            if training:
                ...
                self.state['logger'].print_meter(mode="Train", iepoch=self.state['epoch'])
                self.state['logger'].reset_meter(mode="Train", iepoch=self.state['epoch'])
            else:
                print('Test: \t Loss {loss:.4f}\t Prec@1 {top1:.3f}'.format(
                    loss=loss, top1=top1))
                self.state['logger'].print_meter(mode="Test", iepoch=self.state['epoch'])
                self.state['logger'].reset_meter(mode="Test", iepoch=self.state['epoch'])
    ...
    def on_end_batch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        # measure accuracy
        self.state['classacc'].add(
            self.state['output'].data, self.state['target'])
        ...
        if display and self.state['print_freq'] != 0 and self.state['iteration'] % self.state['print_freq'] == 0:
            ...
            batch_time = self.state['batch_time'].value()[0]
            data_time = self.state['data_time'].value()[0]
            # print('sssssss',self.state['print_freq'])
            if training:
                print('Epoch: [{0}][{1}/{2}]\t'
                      'Time {batch_time_current:.3f} ({batch_time:.3f})\t'
                      'Data {data_time_current:.3f} ({data_time:.3f})\t'
...
class MultiLabelMAPEngine(Engine):
    ...
    def on_end_batch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        # measure mAP
        print(self.state['output'].data,self.state['target_gt'])
