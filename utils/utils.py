...
def save_checkpoint(model, args, state, is_best, filename='checkpoint.pth.tar'):
        if args.save_model_path is not None:
            filename_ = filename
            filename = os.path.join(args.save_model_path, filename_)
        ...
        if is_best:
            ...
            if args.save_model_path is not None:
                filename_best = os.path.join(args.save_model_path, filename_best)
            shutil.copyfile(filename, filename_best)
            if args.save_model_path is not None:
                if self._state('filename_previous_best') is not None:
                    os.remove(self._state('filename_previous_best'))
                filename_best = os.path.join(args.save_model_path, 'model_best_{score:.4f}.pth.tar'.format(score=state['best_score']))
                shutil.copyfile(filename, filename_best)
...
class Warp(object):
    """wrap the size of image and return a wrapper
    input: size eg.(224,224)
    output: (size={size}, interpolation={interpolation})"""
